...
public class AgreementPdfGenerator {
    ...
    private void pageTwo(PDDocument doc)throws Exception{
        PDPage page=new PDPage(PDRectangle.A4);doc.addPage(page);try(PDPageContentStream cs=new PDPageContentStream(doc,page)){
            ...
            top=heading(cs,"8. OBLIGATIONS OF THE CONSULTANT",top+3);bullet(cs,"Use reasonable efforts and professional diligence to identify suitable opportunities matching the Candidate’s profile.",top);bullet(cs,"Coordinate interview scheduling and communication between the Candidate and prospective employers.",top);
        }
    }
